...
class Akwam:
    ...
    def grapper(self, cat: str):
        ...
        while True:
            pages_count = int(
                input(
                    f"{Fore.GREEN}[?]{Style.RESET_ALL} How Many Pages Do You Want To show ? |There are [{pages_count_all}] pages| "
                ).strip()
            )
    ...
    def searcher(self, search_Quiery: str, category: str = None):
        """This Function Will apply Search quiery and get
        the links and title of akwam media
        optional argument is category you can spacify which category you want to print the results from
        ['series' , 'movie' , 'game']
        """
        ...
        for i in range(1, pages_num + 1):
            ...
            if len(search_items) == 0:
                ...
                for n, i in enumerate(search_items):
                    Title = i.select_one(
                        "div.entry-body.px-3.pb-3.text-center > h3 > a"
                    ).text.strip()
                    Link = i.select_one(
                        "div.entry-body.px-3.pb-3.text-center > h3 > a"
                    ).get("href")
    ...
    def __ads_bybasser(self, url):
        ...
        return soup_i.select_one(
            "body > div.site-container > div.page-redirect > div > div > div > p > a"
        ).get("href")

    def fetcher(self, url):
        "This Function Will return{title , type,etc..} and direct Download links for this media"
        ...
        if category == "games":
            ...
            Title = (
                item_soup.select_one(
                    "body > div.site-container > div.page.page-movie.page-film > div.movie-cover.mb-4.without-cover > div > div > div.col-lg-7.pr-lg-4.col-md-5.col-sm-8.mb-4.mb-sm-0.px-4.px-sm-0 > h1"
                )
                .text.strip()
                .title()
            )
            idmb_rate = item_soup.select_one(
                "body > div.site-container > div.page.page-movie.page-film > div.movie-cover.mb-4.without-cover > div > div > div.col-lg-7.pr-lg-4.col-md-5.col-sm-8.mb-4.mb-sm-0.px-4.px-sm-0 > div.font-size-16.text-white.mt-2.d-flex.align-items-center > span.mx-2"
            ).text.strip()
            try:
                year_production = (
                    item_soup.select_one(
                        "body > div.site-container > div.page.page-movie.page-film > div.movie-cover.mb-4.without-cover > div > div > div.col-lg-7.pr-lg-4.col-md-5.col-sm-8.mb-4.mb-sm-0.px-4.px-sm-0 > div:nth-child(7) > span"
                    )
                    .text.strip()
                    .split(":")[1]
                )
            ...
            # _________________________________________________Series
            if category == "series":

                episodes_urls = [
                    i.get("href")
                    for i in item_soup.select(
                        "#series-episodes > div > div > div > h2 > a"
                    )
                ]
                print(
                    f"{Fore.GREEN}[*]{Style.RESET_ALL}The Requested Series Title is : {Title} ,\nThe Idmb rate is {idmb_rate} \n episodes Count {len(episodes_urls)}"
                )

                episodes_D_links = []
                for i, item in enumerate(episodes_urls):
                    D_links = self.ID_links_gen(item)
                    episodes_D_links.append(
                        {'index': i + 1, 'download_links': D_links})
                result = {
                    "Title": Title,
                    "IDMB Rate": idmb_rate,
                    "Year Of Production": year_production,
                    f"{Fore.GREEN}[*]{Style.RESET_ALL} Download Links\n": episodes_D_links,
                }
                first_links = []
                with open(f'{Title}.txt', 'w') as file:
                    for episode in episodes_D_links:
                        first_link = episode.get('download_links')[
                            0].get('link')
...
# __________________________________________________________________
    ...
    def ID_links_gen(self, url):
        ...
        for i in links_tags:
            ...
            D_links.append(
                {'size': size, 'link': self.__ads_bybasser(indirect_link)})
